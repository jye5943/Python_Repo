...
# 수정 날짜 :   2019.05.17
# 추가 내용 :
# - Block class 에 merkleHash 추가.
# - calculateMerkleHash function 추가. txData.csv에서 column[0]의 값이 0인 row를 모두 불러와
#   fee를 기준으로 정렬 및 str로 변환하여 rcGetMerkleHash 에 strlist를 던져준다.
# - rcGetMerkleHash(재귀함수) function 추가. txData를 str으로 cast한 strlist를 순차적으로 2개씩 끊어 hash를 만들고
#   최종적으로 merkleHash를 생성하여 return 한다.
# 수정 내용 :
#  -block class에 변수가 추가됨으로써 수정해야할 모든 부분 수정.
#  -Genesis block 생성시에도 txData에 거래내역이 추가 되게끔 수정.
########################################################################################################################
import hashlib
import time
import csv
...
class txData:
    ...
    def __init__(self, commitYN, sender, amount, receiver, fee, uuid):
...
def generateGenesisBlock():
    ...
    print("time.time() => %f \n" % timestamp)
    # hash생성
    merkleHash = calculateMerkleHash(g_txFileName)
...
#-----------------------------------------V01 : 완
#머클해쉬추출 
def calculateMerkleHash(txDataPath) :
    print("Extracting txData.............")

    #최종 반환할 해쉬
    resultHash = ""
    #txData 문자열 리스트
    strTxDataList = []
    #정렬된 50개의 txData 객체 리스트
    sortedTxDataList = []
    # txData.csv 오픈
    importedTxList = readTx(txDataPath)

    #가져온 거래의 개수가 50보다 크면, 수수료 기준으로 정렬하고 큰 수수료부터 가져온다.
    if len(importedTxList) > 0:
        #어떤 거래내역의 수수료가 가장 큰 지 모르기 때문에 일단 다 가져와야한다.
        if len(importedTxList) > g_maximumGetTx :
            #스왑으로 정렬. 리스트 자체를 바꿔줘야 제대로 된 정보를 가져올 수 있음.
            for i in importedTxList :
                for j in importedTxList :
                    if j.fee > i.fee :
                        tempList = i
                        i = j
                        j = tempList
        ...
        print("check sort]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]")
        for t in importedTxList :
            print(t.__dict__)
        # 높은 순으로 50개를 끊는다.
        count = 0
        for k in importedTxList :
            if count >= g_maximumGetTx :
                break
            sortedTxDataList.append(k)
            count = count + 1

        print(len(sortedTxDataList))
        #해쉬 하기위해 문자열로 변환
        for m in sortedTxDataList:
            print(m.__dict__)
            transaction = "[" + m.uuid + "]" "UserID " + m.sender + " sent " + m.amount + " bitTokens to UserID " + m.receiver + " fee " + m.fee + ". " 
            print(transaction)
            strTxDataList.append(transaction)

        print("hash merkling..................")
        #문자열 리스트를 머클트리 재귀함수에 던져준다.
        return rcGetMerkleHash(strTxDataList)
    else :
        print("No exist unmined txData")
        return ""
...
# -----------------------------------------V01 : 완
# blockData를 merkleHash로 변경하여 블록을 생성하게 바꾼다.
def generateNextBlock(blockchain,blockData, timestamp, proof):
    ...
    return Block(nextIndex, previousBlock.currentHash, nextTimestamp, blockData, nextHash, proof, merkleHash)
...
def updateTx(blockData):
    ...
    with open(g_txFileName, 'r') as csvfile, tempfile:
        ...
        # 각 행들을 순차적으로 읽어들여와
        for row in reader:
            # 거래한 블록체인의 해쉬코드(matchList로 해당 블록체인의 data 찾아낸 해쉬코드)에서 가 "txdata.csv"(row[4])에도 존재한다면
...
def writeTx(txRawData):
    ...
    for txDatum in txRawData:
        txList = [txDatum.commitYN, txDatum.sender, txDatum.amount, txDatum.receiver, txDatum.fee, txDatum.uuid]
...
# txdata.csv파일을 읽어와 행 단위로 txData객체에 담고, 그 객체들을 다시 메소드
...
# "txdata.csv"파일을 읽어와 거래내역을 전체를 string변수화 하여 반환한다.
def getTxData():
    strTxData = ''
...
def newtx(txToMining):
    ...
    for line in txToMining:
        tx = txData(0, line['sender'], line['amount'], line['receiver'], line['fee'], uuid.uuid4())
        newtxData.append(tx)
